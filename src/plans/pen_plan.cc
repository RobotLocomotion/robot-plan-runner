...
PenPlan::PenPlan(
    ...
    : PlanBase(plant), quat_traj_(std::move(quat_traj)), xyz_traj_(std::move(xyz_traj)),
      frame_L7_(plant->GetFrameByName("iiwa_link_7")), x_TC_desired_(x_desired) {
  ...
  rpe_sub_ = std::make_unique<drake::lcm::Subscriber<drake::lcmt_robot_state>>(
      owned_lcm_.get(), "X_TC");
  sub_thread_ = std::thread(&PenPlan::PoseIo, this);
...
void PenPlan::Step(const State &state, double control_period, double t,
                           Command *cmd) const {
  // Forward kinematics.
  ...
  if (controller_active_) {
    ...
    double error = x_TC_now_ - x_TC_desired_;
    x_previous_ = x_previous_ + kp_ * error;

    X_WT_new = X_WT_desired * (
      drake::math::RigidTransformd(
        drake::math::RollPitchYaw(
          Eigen::Vector3d(0, 0, 0)),
          Eigen::Vector3d(x_previous_, 0, 0)));

  } else {
    X_WT_new = X_WT_desired;
  }

  std::cout << x_TC_desired_ << std::endl;
