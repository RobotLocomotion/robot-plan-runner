...
IiwaPlanFactory::MakePlan(const drake::lcmt_robot_plan &msg_plan) const {
  ...
  } else if (first_joint_name == "qw_pen") {
    return MakePenPlan(msg_plan);
  }
...
/*
std::unique_ptr<PlanBase> IiwaPlanFactory::MakeScrewdriverPlan(
    const drake::lcmt_robot_plan &msg_plan) const {
  int n_knots = msg_plan.num_states;
  int n_q = msg_plan.plan.at(0).num_joints;
  MatrixXd q_knots(n_q, n_knots);
...
std::unique_ptr<PlanBase> IiwaPlanFactory::MakePenPlan(
    const drake::lcmt_robot_plan &msg_plan) const {
  int n_knots = msg_plan.num_states - 1;
  ...
  const double x_pen = msg_plan.plan.at(0).joint_position[7];
  ...
  return std::make_unique<PenPlan>(
      std::move(quat_traj), std::move(xyz_traj), plant_.get(),
      config_["control_period"].as<double>(), nominal_joint, x_pen);
}