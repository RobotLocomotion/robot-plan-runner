#include "bimanual_iiwa_plan_factory.h"
...
BimanualIiwaPlanFactory::BimanualIiwaPlanFactory(const YAML::Node &config)
    : config_(config) {
...
std::unique_ptr<PlanBase> BimanualIiwaPlanFactory::MakePlan(
...
std::unique_ptr<PlanBase> BimanualIiwaPlanFactory::MakeJointSpaceTrajectoryPlan(
    const drake::lcmt_robot_plan &msg_plan) const {
...
// BimanualIiwaPlanFactory::MakeTaskSpaceTrajectoryPlan(
//     const drake::lcmt_robot_plan &msg_plan) const {
...
//   const auto Q_ET =
//       Quaterniond(q_xyz_ET[0], q_xyz_ET[1], q_xyz_ET[2], q_xyz_ET[3]);
//   const auto p_EoTo_E = Eigen::Vector3d(q_xyz_ET[4], q_xyz_ET[5],
//   q_xyz_ET[6]); const auto X_ET = drake::math::RigidTransform<double>(Q_ET,
//   p_EoTo_E);
...
//   Eigen::VectorXd nominal_joint = Eigen::Map<Eigen::VectorXd,
//   Eigen::Unaligned>(
