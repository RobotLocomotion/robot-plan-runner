...
void TaskSpaceTrajectoryPlan::Step(const State &state, double control_period, double t,
                          Command *cmd) const {
  ...
  const drake::multibody::Frame<double>& frame_E = plant_->GetFrameByName("iiwa_link_7");
  ...
  drake::math::RigidTransformd X_WE_desired(
    drake::math::RotationMatrixd(quat_traj_.value(t)), xyz_traj_.value(t));

  DifferentialInverseKinematicsResult result = DoDifferentialInverseKinematics(
    *plant_, *plant_context_, X_WE_desired.GetAsIsometry3(), frame_E, *params_);
  ...
  if (result.status != DifferentialInverseKinematicsStatus::kSolutionFound) {
    ...
    std::cout << "DoDifferentialKinematics Failed to find a solution." << std::endl;
  }
  else {
